...
The goal of this chapter is to explain the transition from the classical theory of stacks to the homotopical one. The main reference is \citesimpsheaf{hollander}. This chapter is organized as follows:
\begin{enumerate}
...
The reader is supposed to already have a familiarity with Grothendieck topologies. I strongly recommend the book of MacLane and Moerdijk \citesimpsheaf[Ch. III]{sheaves} for a clear exposition of this theory. A neat treatment can be also found in the article \citesimpsheaf[Ch. II]{vistoli}.
...
Our exposition will follow \citesimpsheaf[Ch. III]{vistoli}, with some integration from \citesimpsheaf[Exposé VI]{sga1}. I strongly recommend the reader to think to fiber bundle (vector bundle if he prefers) while reading these notes.
...
\]
For each $\eta \in \Ob(\mathcal F_U)$ choose a cartesian arrow $\phi \colon \xi \to \eta$ lying over $f$. Then the pair $(\phi, \mathrm{id}_\eta)$ is a universal arrow from $\Psi_f$ to $\eta$.\footnote{This is the very definition of cartesian arrow, but remember also that all the arrows in $\mathcal F_U$ maps to the identity of $U$ via $p$.} It follows from the standard characterization of adjunctions (cfr. \citesimpsheaf[Theorem IV.1.2.(iv)]{cwm} that $\Psi_f$ has a right adjoint
\[
...
\]
In this construction we are hiding the axiom of choice. We know from \citesimpsheaf[Theorem IV.1.2.(iv)]{cwm} that to construct the adjoint $f^*$ one has only to choose universal arrows for every object in $\mathcal F_U$. If we make this choice for every arrow $f \colon V \to U$ we obtain what is traditionally called a \emph{cleavage}:
...
(U,x) \to (V,y)
\]
to be a pair $(f, \tau)$ where $f \colon U \to V$ is an arrow in $\mathcal C$ and $\tau \colon x \to \Phi(f)(y)$ is an arrow in $\Phi(U)$. The details for the construction can be found in \citesimpsheaf[Ch 3.1.3]{vistoli}.
\end{proof}
...
\begin{proof}
Straightforward (for details, see \citesimpsheaf[Proposition 3.22]{vistoli}.
\end{proof}
...
\begin{proof}
Straighforward (see \citesimpsheaf[Proposition 3.25]{vistoli} for details).
\end{proof}
...
There are several ways to define descent data and descent condition. We will follow the exposition given in \citesimpsheaf[Ch. 4]{vistoli}.
...
\]
It is straighforward to check that this is a bijection (cfr. \citesimpsheaf[Prop. 2.39]{vistoli}).
\end{proof}
...
Others description are possible. See \citesimpsheaf[Ch. 4.1.2]{vistoli} for a detailed explanation.
...
\]
where $\mathcal O_R$ is a sheaf of rings and $\mathcal F$ is a $\mathcal O_R$-module (the reader not at ease with Algebraic Geometry might would like to check on \citesimpsheaf[Prop. II.5.8.(a)]{hartshorne}).
\end{proof}
...
\]
at least without additional hypothesis on $f$ (e.g. quasi-compact and separated, see \citesimpsheaf[Prop. II.5.8.(c)]{hartshorne}).
\end{rmk}

The adjointness allows to construct the required isomorphisms $\alpha_{f,g}$ and $\varepsilon_X$. For the details, see \citesimpsheaf[Ch. 3.2.1]{vistoli}. Let now
\[
...
\begin{proof}
See \citesimpsheaf[Thm. 4.23]{vistoli}.
\end{proof}
...
\begin{enumerate}
\item proper morphisms are stable under base change. See \citesimpsheaf[Prop. 3.3.16.(c)]{liu};
\item smooth morphisms are stable under base change. See \citesimpsheaf[Prop 4.3.38]{liu};
\item morphisms of finite type are stable under base change. See \citesimpsheaf[Prop 3.2.4.(c)]{liu};
\item if $f \colon X \to Y$ is a smooth morphism and $x \in X$ is a point, the relative dimension of $f$ at $x$ can be computed as the rank of $\Omega^1_{X/Y}$ at $x$; see \citesimpsheaf[Def. 6.4.10]{liu}.
\end{enumerate}
...
B' := B \otimes_A A'
\]
so that \citesimpsheaf[Prop 6.1.8.(a)]{liu} implies that
\[
...
\begin{proof}
See \citesimpsheaf[Thm. 4.38]{vistoli}.
\end{proof}
...
This is effectively done in \citesimpsheaf{hollander}. Here, we start with a more general point of view, and we will go back to the original problem only at the end. Before starting, let's sketch what our journey will be: recall from Theorem \ref{thm groupoids nullification} that we have an adjoint pair
\[
...
\]
It's an exercise in formalism to check that the axioms of Definition \ref{def enriched category} are satisfied. The reader can find all the verifications in \citesimpsheaf[Lemma II.2.4]{gj} in a more general form.
\end{proof}
...
\begin{rmk}
This model structure for presheaves of simplicial sets goes back to the work of Heller \citesimpsheaf{heller}.
\end{rmk}

However, when we consider a site $(\mathcal C, J)$, we can define a model structure over $\mathrm{sPSh}(\mathcal C)$ taking in account the topology $J$. The idea to do that goes back to Jardine (\citesimpsheaf{jardinepresheaves}), at least to the best of my knowledge, and relies on the notion of sheaf of homotopy groups.
...
Following Jardine \citesimpsheaf{jardinepresheaves} we give the following definition:
...
\begin{proof}
See \citesimpsheaf[Thm. 2.3]{jardinepresheaves}.
\end{proof}
...
\begin{proof}
See \citesimpsheaf[Prop. 7.2]{weakequivalences}.
\end{proof}

\subsection{Characterization of fibrant objects}

We want to relate the notion of ``being fibrant'' and ``satisfy descent condition''. First of all, we will need to develop the machinery of hypercovers. Our main reference is \citesimpsheaf{hypercover}.
...
\begin{proof}
See \citesimpsheaf[Prop. 6.6]{hypercover}.
\end{proof}
...
\begin{proof}
See \citesimpsheaf[Thm. 6.2]{hypercover}.
\end{proof}
...
\begin{proof}
See \citesimpsheaf[Lemma 4.4]{hypercover}.
\end{proof}
...
\begin{proof}
See \citesimpsheaf[Cor. 7.1]{hypercover}.
\end{proof}
...
\begin{proof}
See \citesimpsheaf[Thm. 7.1]{hollander}.
\end{proof}
...
\begin{proof}
See \citesimpsheaf[Prop. 7.5]{hollander}
\end{proof}
...
\begin{proof}
See \citesimpsheaf[Cor. 8.10]{hollander}.
\end{proof}
...
\begin{proof}
See \citesimpsheaf[Thm 4.3]{hollander}.
\end{proof}
...
\]
See \citesimpsheaf[Ch. 3.6.2]{vistoli} for a proof.
\end{rmk}
...
The main reference is \citesimpsheaf[Ch. VI]{sga1}.
