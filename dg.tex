...
\begin{definition}
  The \emph{graded cohomology category}~$\HH^\bullet(\mathcal{C})$ is the category with~$\Ob(\HH^\bullet(\mathcal{C}))\coloneqq\Ob(\mathcal{C})$ but we take\fixthis{(question) might it be better to use the terminology (graded) (co)homology category?}
  \begin{equation}
    \Hom_{\HH^\bullet(\mathcal{C})}(X,Y)^\bullet\coloneqq\HH^\bullet\left( \Hom_{\mathcal{C}}(X,Y)^\bullet \right).
  \end{equation}
\end{definition}
