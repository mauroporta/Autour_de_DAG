...
\begin{theorem}[\cite{tabuada-2}]
  ...
  If we take take the Morita equivalence as weak equivalences, the quasi-fibrations as fibrations, and the dg~functors satisfying the right lifting property with respect to the trivial fibrations as cofibrations we obtain a model category structure on~$\dgCat_k$.
\end{theorem}
...
Recall that there exists a closed monoidal structure on~$\dgCat_k$. Unfortunately, this structure does not descend to the homotopy category, as the tensor product of two cofibrant dg~categories is not necessarily cofibrant. So in order to prove the existence of a closed monoidal structure on~$\Ho(\dgCat_k)$ one needs to do some work. On the other hand, almost by the very definition of the Morita equivalences the internal Hom descends to the homotopy category with respect to this structure. The statements in this section will nevertheless use the classical model category structure.
\begin{theorem}
...
It is also possible to introduce a simplicial structure on~$\dgCat_k$, which moreover descends to a~$\Ho(\sSet)$\dash structure on~$\Ho(\dgCat_k)$. The internal Hom is compatible with this structure.
\begin{corollary}
...
\begin{equation}
  \mathcal{C}\to\RRRHHom(\Delta_k^1,\mathcal{C})\eqqcolon\Mor_\dg(\mathcal{C}).
\end{equation}
This morphism yields the functorial cone construction. For a non-dg category~$\mathcal{A}$ one has its category of morphisms~$\Mor(\mathcal{A})$, and using the dg~enriched category of morphisms one can obtain a natural functor
\begin{equation}
  \HH^0(\Mor_\dg(\mathcal{C}))\to\Mor(\HH^0(\mathcal{C}))
\end{equation}
which is in general essentially surjective and full. But it is not faithful, which corresponds to the failure of the functoriality of the cone construction. For a more detailed discussion, see \cite[section 5.1]{lnm2008}.
